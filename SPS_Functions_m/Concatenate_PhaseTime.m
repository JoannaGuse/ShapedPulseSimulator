function [pulse ] = Concatenate_PhaseTime(pulse)
...
%[p2]=Concatenate_PhaseTime(pulse);
...
p2.phi=-p1.phi;
p2.f_mod=-p1.f_mod;
p2.Bx=p2.env.*cos(p2.phi);
p2.Bx=p2.env.*sin(p2.phi);

%% make third pulse R1
p3=p1;
p3.phi0=pi/2;
p3.t=p1.t;
p3.env=flipud(p1.env);
p3.phi=flipud(p1.phi);
p3.f_mod=flipud(p1.f_mod);
p3.Bx=p3.env.*cos(p3.phi+p3.phi0);
p3.Bx=p3.env.*sin(p3.phi+p3.phi0);
